...
import ExpandingLoginButton from "./CompExpandBtn";
...
const NavBar: FC<NavBarProps> = ({ onMenuClick }: NavBarProps) => {
  ...
  return (
    <>
      {/* Fixed height header */}
      <header className="sticky top-0 z-50 w-full h-16 border-b border-gray-200 bg-white/90 backdrop-blur">
        <nav className="h-full max-w-[1920px] mx-auto px-4 flex items-center justify-between">
          {/* Logo/Home Link - Fixed width */}
          ...
          <div className="flex-1 max-w-md mx-4">
            ...
            {/* Search Modal/Popup */}
            {isSearchOpen && (
              <div className="absolute top-16 left-0 right-0 bg-white shadow-lg rounded-lg mt-1 p-4">
          ...
          <div className="w-48 flex-shrink-0 flex items-center justify-end space-x-4">
            {/* Theme Toggle */}
            <button
            ...
            {/* Login Button Container - Positioned absolutely */}
            <div className="absolute top-[11px] right-2.5 z-50">
              <ExpandingLoginButton />
            </div>
          </div>
        </nav>
      </header>
